(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    if(Array.isArray(collection)){
      ...
      _.each(collection, function(elem, i, collection){
        acc.push(iterator(elem, i, collection));

      })
    } else {
      var acc = {};

      _.each(collection, function(elem, key, collection){
        acc[key] = iterator(elem, key, collection);
      })
    }
    return acc;
  };
  ...
  _.shuffle = function(array) {
    ...
    for(var i=newArr.length-1; i>=0; i--){
      var random =  Math.floor(Math.random() * (array.length - i))+i;
